...
def train(mode,mc=False):
    ...
    gen_train = aug_generator(X_tr,Y_tr,batch_size=3,flip_axes=['x','y'])
...
def eval_mc(mode,sample_times=20):
    ...
    for batch_id in tqdm(range(X_ts.shape[0] // batch_size)):
        # initialize our predictions
        Y_ts_hat = np.zeros(shape=(sample_times,X_ts.shape[0],target_height,target_width,1),dtype='float32')
        #print(Y_ts_hat.shape) 
        ...
        for sample_id in range(sample_times):
            ...
            Y_ts_hat[sample_id] = model.predict(X_ts, batch_size)
        # average over all passes
        prediction = Y_ts_hat.mean(axis=0)       
...
#   %matplotlib inline
  ...
  Ndatapoints = 40 #number of datapoints in the test set
...
Ndata=1 #number of modes(percentages%)
...
Ndatapoints = 20 #number of datapoints in the test set
Nmodels = 10 #number of models trained
Ndata=5
X = np.zeros((Ndatapoints*Nmodels,Ndata),dtype='float')#initialize to zeros
print(np.median(dice[1]),np.median(dice[2]))
